...
| 3 | **Confident.** The claim of the whole project | "This house is an **MCP server**. The answer is the host's **own** instructions — **not** a model's guess." |
| 4 | Brisk, practical. Three verbs, three beats | "Say the coffee's out. It **checks** the cupboard, **orders** a refill from the host's supplier, and **tells** the host." |
| 5 | **Build, then slow right down from "Tomorrow".** Best thing in the demo | "Report a leak, and it opens a **ticket**, picks the host's **plumber**, and proposes a **time**. Say yes, and it's **booked**. **Tomorrow**, the house **still knows** who is coming, and why." |
| 6 | **Flat and factual. Do not sell this one** — the flatness is the credibility | "**Every** one of those answers was a **real** MCP tool call. This drawer **shows** them. **Nothing** is mocked." |
| 7 | Light, quick, almost a throwaway | "**One** server, **three** houses — each with its **own** calendar." |
| 8 | Deliberate, explaining. Slow. The two "cannot"s are the rule | "In this one, the guest leaves **tomorrow**, and the next arrives the **day after**. So a repair reported today **cannot** land tomorrow, and it **cannot** land the day after either." |
| 9 | The payoff. Land it, then stop | "It lands **three days out** — the **first** day nobody is checking in or out." |
| 10 | Warm. The only line that should sound pleased | "**Live**, **open source**, and **honest** about its limits. The house **explains itself**." |

A sentence with no stressed word sounds flat however certain you feel, so the bold is the whole
trick — hit those, let everything else fall away, and the ups and downs take care of themselves.

---

## What can and cannot be fixed afterwards

Fixable in the mix: levels (a quiet line lifted to match the rest), warmth and room noise, the
gaps between lines, and slowing one line by ~5% if it came out rushed. **A single line can also be
re-recorded on its own and spliced in** — one bad line does not cost you the take.

Not fixable: intonation. Rise and fall inside a sentence is per-syllable, and anything that fakes
it sounds synthetic — the same reason a TTS voice was rejected. If a line lands flat, say that
line again rather than hoping the mix rescues it.
